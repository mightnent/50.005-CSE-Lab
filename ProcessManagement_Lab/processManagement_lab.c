...
void setup(){
    ...
    //          f. Create number_of_processes semaphor          es of value 0 each to protect each job struct in the shared memory. Store the returned pointer by sem_open in sem_jobs_buffer[i]
    //          g. Return to main
    ...
    sem_t* sem_gd_status = sem_open(sem_global_data, O_CREAT | O_EXCL , 0644, 1);

    while ( sem_gd_status == SEM_FAILED) {
        sem_unlink(sem_global_data);
        sem_gd_status = sem_open(sem_global_data, O_CREAT | O_EXCL , 0644, 1);
    }
    
    // create shared memory for number of proceseses 
